...
$SystemData = @{
    Stanton = "ArcCorp; Crusader; Hurston; Microtech"
}
...
$JavelinDaymar.Location = new-object System.Drawing.Point(5,160) 
...
while($StartNavigation) {
    ...
    elseif($Global:PlanetaryPoi){
        #GET UTC SERVER TIME
        $UTCServerTime = [DateTime](Get-Date).ToUniversalTime()                                                   # GET CURRENT TIME IN UTC
        $SimulationServerStartTime = [DateTime] "01.01.2950 00:00"                                                # SET STARTTIME SIMULATION STARCITIZEN
        $SimulationUTCStartTime = [DateTime]"01.01.2020 00:00"                                                    # SET STARTTIME SIMULATION UTC
        ...
        # DESTINATION = Shubin Mining Facility SCD-1
        # ANGLE Turret = 24
        # ANGLE Turret = 36
        # ANGLE DISTANCE = 378,7 km
        ...
        $debug = $true
        if ($debug){
            $UTCServerTime = [DateTime]::ParseExact('28.12.2020 21:30:20','dd.MM.yyyy HH:mm:ss',$null)
            $ClipboardContent = "Coordinates: x:-18930741401.188889 y:-2609779521.318735 z:-869.386052"
        }
    ...
    if (!$debug){$ClipboardContent = (Get-Clipboard).split(":").Split(" ")}
    #$ClipboardContent = (Get-Clipboard).split(":").Split(" ") #GET CURRENT COORDS FROM CLIPBOARD
    
    if ($ScriptLoopCount -lt 1){
        if (!$debug){$ClipboardContent = ("Coordinates: X:0 y:0 z:0").split(":").Split(" ")}
