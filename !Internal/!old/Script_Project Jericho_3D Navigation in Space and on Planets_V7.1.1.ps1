...
while($StartNavigation) {
    ...
    $WgsLatitude = [math]::Round([math]::ASin($CurrentPlanetaryZCoord*1000 / $RadialDistance) * 180 / [Math]::PI,6)
...
#>
    ...
    #Write each update into history file
    if($script:CurrentDetectedObjectContainer){
        #$Playername,$($DateTime.ToString('HH:mm:ss:ffff'))
        #$HistoryContent = "$CurrentPlanetaryXCoord,$CurrentPlanetaryYCoord,$CurrentPlanetaryZCoord"
        ### Convert Lat into meters for scale ###
        #FIRST CALC THE CIRCUMFERENCE
        $LatCircum = ([Math]::PI * [Math]::Pow(($CurrentDetectedBodyRadius + $WgsHeight),2))/360
      
        
        $WgsLatitude * $CurrentDetectedBodyRadius + $WgsHeight
        $WgsLongitude
        $HistoryContent = ",,$WgsHeight,$Playername"
        $HistoryContent  >> 'Data\history_local.csv'
    } #IF ON A PLANET
    else {
