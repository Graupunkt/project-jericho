...
while($StartNavigation) {
    ...
    #GET DESTINATION COORDINATES FROM HASTABLES, FILTER FOR CURRENT DESTINATION
        ...
        $DestPlanet = $SelectedDestination."ObjectContainer"
    }
    ...
    if($script:CurrentDetectedObjectContainer -eq $DestPlanet){
    ...
    $WgsLatitude = [math]::Round([math]::ASin($CurrentZPosition / $RadialDistance) * 180 / [Math]::PI,3)
    $WgsLongitude = [math]::Round([math]::Atan2($CurrentXPosition, $CurrentYPosition) * 180 / [Math]::PI,3) * -1

    $WGSSpacing = 20
    $WGS = "Planet/Moon : ".PadRight($WGSSpacing),"${VTDarkgray}Lat: ${VTDefault}$WgsLatitude°".PadRight($WGSSpacing+4),"${VTDarkgray}Long: ${VTDefault}$WgsLongitude°".PadRight($WGSSpacing+4),"${VTDarkgray}Height: ${VTDefault}$WgsHeight"
