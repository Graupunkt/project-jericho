...
if ($ScriptLoopCount -eq 0){
    ...
    $WindowSizeY = 35
    $WindowSizeX = 85
    $MaxX = $ScreenResolution[0].WorkingArea.Width -650 
    #$MaxY = $ScreenResolution[0].WorkingArea.Height
    #Move-Window $MaxX 25 | Out-Null
...
while($StartNavigation) {
    ...
    #Calculate Orbital Marker Distances
    #First Value is in meters
    #Final Value is rounded in kilometers
    $OM1 = [math]::Pow(([math]::Pow("$CurrentPlanetaryXCoord","2") + [math]::Pow("$CurrentPlanetaryYCoord","2") + [math]::Pow($CurrentPlanetaryZCoord-$CurrentDetectedOCRadius,"2")),1/2)
    $OM2 = [math]::Pow(([math]::Pow("$CurrentPlanetaryXCoord","2") + [math]::Pow("$CurrentPlanetaryYCoord","2") + [math]::Pow($CurrentPlanetaryZCoord-(-$CurrentDetectedOCRadius),"2")),1/2)
    $OM3 = [math]::Pow(([math]::Pow("$CurrentPlanetaryXCoord","2") + [math]::Pow($CurrentPlanetaryYCoord-$CurrentDetectedOCRadius,"2") + [math]::Pow($CurrentPlanetaryZCoord,"2")),1/2)
    $OM4 = [math]::Pow(([math]::Pow("$CurrentPlanetaryXCoord","2") + [math]::Pow($CurrentPlanetaryYCoord-(-$CurrentDetectedOCRadius),"2") + [math]::Pow($CurrentPlanetaryZCoord,"2")),1/2)
    $OM5 = [math]::Pow(([math]::Pow($CurrentPlanetaryXCoord-$CurrentDetectedOCRadius,"2") + [math]::Pow("$CurrentPlanetaryYCoord","2") + [math]::Pow($CurrentPlanetaryZCoord,"2")),1/2)
    $OM6 = [math]::Pow(([math]::Pow($CurrentPlanetaryXCoord-(-$CurrentDetectedOCRadius),"2") + [math]::Pow("$CurrentPlanetaryYCoord","2") + [math]::Pow($CurrentPlanetaryZCoord,"2")),1/2)
    #$FinalOM1 = [math]::Round(($OM1/1000),2)
    #$FinalOM2 = [math]::Round(($OM2/1000),2)
    #$FinalOM3 = [math]::Round(($OM3/1000),2)
    #$FinalOM4 = [math]::Round(($OM4/1000),2)
    #$FinalOM5 = [math]::Round(($OM5/1000),2)
    #$FinalOM6 = [math]::Round(($OM6/1000),2)
    #$OrbitalMarkers = "OM1: $FinalOM1 km, OM2: $FinalOM2 km, OM3: $FinalOM3 km, OM4: $FinalOM4 km, OM5: $FinalOM5 km, OM6: $FinalOM6 km, "
