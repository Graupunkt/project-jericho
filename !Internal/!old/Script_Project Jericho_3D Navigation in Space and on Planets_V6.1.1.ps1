...
#OM3 Auditing
# 19.02.2021 = Pisces 1m = -0.018

#WOLF POINT AID SHELTER - at the end of the stairs - Standing on the Caution Sign
# 19.02.2021 20:55 = -4.422;279.071;95.727 | -4.419;279.071;95.727 | -4.416;279.071;95.727
...
$ObjectContainerData = @{                                           #If x has a negative offset, raise Rotation Adjustment     OrbitalMarkerRadius is also used to determine if the player is within the object container for now
    ...
    Daymar              = "-18930539540;        -2610158765;        0;                  2.4800000;      30.02790;               430026;             Stanton"   #Stanton2b
...
$OcCsvPath = "C:\Users\marcel\Desktop\StarCitizen Tools\Projekt Jericho (3D Navigation)_V6\data\Script_Project Jericho_Values for Object Containers.csv"
...
#$OcCsvPath = "data\Script_Project Jericho_Values for Object Containers.csv"
$PoiPlanetsCsvPath = "C:\Users\marcel\Desktop\StarCitizen Tools\Projekt Jericho (3D Navigation)_V6\data\Script_Project Jericho_Points of Interest on Planets and ObjectContainers.csv"
...
while($StartNavigation) {
    ...
    #    if($debug){
    #        Write-Host "Coordinates differ from previous ones"
    #}
    ...
    if ($pressed) { 
        ...
        if($script:CurrentDetectedObjectContainer){$PoiToSave = "$SystemName;$CurrentDetectedObjectContainer;CustomType;$PoiName;$CurrentPlanetaryXCoord;$CurrentPlanetaryYCoord;$CurrentPlanetaryZCoord;$DateTime"}
        #IF IN SPACE
        #else {$line_to_write = $NewName + ';in Space;' + $CurrentXPosition + ';' +  $CurrentYPosition + ';' + $CurrentZPosition}
        else {$PoiToSave = "$SystemName;Custom POI Type;$PoiName;$CurrentXPosition;$CurrentYPosition;$CurrentZPosition;$DateTime"}
    ...
    Write-Host "Updated: $($DateTime.ToString('HH:mm:ss')), last update: $LastUpdate, Destination: $DestinationName"
    Write-Host -ForegroundColor Yellow $DateTime.ToString('HH:mm:ss:ffff')
