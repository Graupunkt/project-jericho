...
$DefaultVariables += debug, variable
...
if($debug){
    foreach($Variable in $DefaultVariables){
    Get-Variable -Exclude debug, $DefaultVariables | Clear-Variable
    Get-Variable -Scope GLOBAL | Where-Object -Property Name -ne "DefaultVariables"
    Remove-Module *
    $error.Clear()
}
...
$PreviousZPosition = 0
$ScriptLoopCount = 0
               #Disables clear screen commands and uses hard coded variables
...
$Button.Add_Click({
    ...
    if ($ShubinMiningFacilitySCD1.Checked -eq $true)     {$Global:CurrentDestination = "Shubin Mining Facility SCD-1";$Global:PlanetaryPoi = $true}
    if ($EagerFlatsAidShelter.Checked -eq $true)         {$Global:CurrentDestination = "Eager Flats Aid Shelter";$Global:PlanetaryPoi = $true}
    if ($KudreOre.Checked -eq $true)                     {$Global:CurrentDestination = "Kudre Ore";$Global:PlanetaryPoi = $true}
    if ($WolfPointAidShelter.Checked -eq $true)          {$Global:CurrentDestination = "Wolf Point Aid Shelter";$Global:PlanetaryPoi = $true}
    if ($NuenWasteManagement.Checked -eq $true)          {$Global:CurrentDestination = "Nuen Waste Management";$Global:PlanetaryPoi = $true}
    if ($JavelinDaymar.Checked -eq $true)                {$Global:CurrentDestination = "Javelin (Daymar)";$Global:PlanetaryPoi = $true}
    if ($KudreOreMine.Checked -eq $true)                 {$Global:CurrentDestination = "Kudre Ore Mine";$Global:PlanetaryPoi = $true}
    if ($CustomButton.Checked -eq $true)                 {$Global:CustomCoordsProvided = $true}

    if ($CheckBox1.Checked  -eq $true) {$global:CurrentQuantumMarker += "ArcCorp"}
    if ($CheckBox2.Checked  -eq $true) {$global:CurrentQuantumMarker += "Crusader"}
    if ($CheckBox27.Checked -eq $true) {$global:CurrentQuantumMarker += "Hurston"}
    if ($CheckBox3.Checked  -eq $true) {$global:CurrentQuantumMarker += "Microtech"}
    if ($CheckBox4.Checked  -eq $true) {$global:CurrentQuantumMarker += "Delamar"}
...
while($StartNavigation) {
    ...
    elseif($Global:PlanetaryPoi){
        ...
        $SelectedDestination = $PointsOfInterestOnPlanetsData.GetEnumerator() | Where-Object { $_.Key -eq $Global:CurrentDestination }
        ...
        $FinalPoiCoords = @{}
        $FinalPoiCoords = @{
            "$Global:CurrentDestination" = "$DestCoordDataX;$DestCoordDataY;$DestCoordDataZ"
        }
    ...
    ###############################
        ...
        $SelectedDestination = $PointsOfInterestInSpaceData.GetEnumerator() | Where-Object { $_.Key -eq $Global:CurrentDestination }
        $DestCoordData = $SelectedDestination.Value -Split ";"
    ...
    if ($debug){
        #$ClipboardContent = ("Coordinates: x:-18930262707.175606 y:-2610486156.324543 z:-85.636128").split(":").Split(" ")
    }
    ...
    if($Global:PlanetaryPoi){$FirstCoordArray += $FinalPoiCoords}
    if($Global:PlanetaryPoi){$FirstCoordArray += $FinalPlanetCoords}
