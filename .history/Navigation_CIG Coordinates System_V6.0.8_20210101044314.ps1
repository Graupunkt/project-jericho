...
while($StartNavigation) {
    ...
    $DeltaXY = [math]::Sqrt([math]::pow($DeltaX,2) + [math]::pow($DeltaY,2) + [math]::pow($DeltaZ,2))
    Write-Host -ForegroundColor Red "test"
    
    [math]::Atan(([math]::pow($DeltaZ,2) - [math]::pow($DeltaY,2) + [math]::pow($DeltaX,2)) / (-2 * $DeltaY * $DeltaZ))*180/[System.Math]::PI
