...
$UseTestdata = $false
#$UseTestdata = $true
$UseThirdPartyTools = $false
;#$UseThirdPartyTools = $true
...
if ($UseThirdPartyTools){
    #DETERMINE SCRIPT PATH WITHIN VSCODE, iSE OR POWERSHELL
    if($env:TERM_PROGRAM -eq "vscode"){$ScriptDir = $psEditor.GetEditorContext().CurrentFile.Path -replace '[^\\]+$'}
    if($psISE){$ScriptDir = Split-Path -Path $psISE.CurrentFile.FullPath}
    else{$ScriptDir = $PSScriptRoot}

    if((get-process "Showlocation_Hotkey ALT-R or LEFTCTRL+ALT_RunAsAdmin" -ea SilentlyContinue) -eq $Null){Start-process -FilePath "$ScriptDir\Showlocation_Hotkey ALT-R or LEFTCTRL+ALT_RunAsAdmin.exe"}
    if((get-process "AntiLogoffScript" -ea SilentlyContinue) -eq $Null){Start-process -FilePath "$ScriptDir\Autoit Scripts\AntiLogoffScript.exe"}
}
...
Function Set-WindowSize{
    $size = New-Object System.Management.Automation.Host.Size(100,225)
...
$Form.Size = New-Object System.Drawing.Size(1050,670) 
...
$statusstrip1.Items.Add('© [VORA] Graupunkt, [DSC] Murphy, [IFE] Xabdiben', "" , $clickEvent)
$statusstrip1.Items.Add('Support', "" , $clickEvent)
$StatusStrip.Operation.Text = '© [VORA] Graupunkt, [DSC] Murphy, [IFE] Xabdiben'
$StatusStrip.Operation.Width = 200
$StatusStrip.Operation.Visible = $true

$StatusStrip = Set-StatusStrip -StatusStrip $StatusStrip -Operation "Test" -Progress "test2"
#$StatusStrip = Set-StatusStrip -StatusStrip $StatusStrip -Operation '© [VORA] Graupunkt, [DSC] Murphy, [IFE] Xabdiben'
$Form.Controls.Add($StatusStrip) 
#© Credits [VoRa] Graupunkt , [DSC] Murphy , [IFE] Xabdiben 

### SUPPORT LINK ###
$SupportLink = New-Object System.Windows.Forms.LinkLabel 
#$SupportLink.Location = New-Object System.Drawing.Size(500,730) 
$SupportLink.size = New-Object System.Drawing.Size(250,50)
$SupportLink.LinkColor = "BLUE" 
$SupportLink.ActiveLinkColor = "RED" 
$SupportLink.Text = "https://www.guilded.gg/projectjericho"
#$SupportLink.Add_PreviewMouseDown({[system.Diagnostics.Process]::start('http://www.vmware.com')})
#$SupportLink.Add_MouseLeftButtonUp({[system.Diagnostics.Process]::start('http://www.google.com')})
$SupportLink.add_Click({[system.Diagnostics.Process]::start('https://www.guilded.gg/projectjericho')}) 
$Form.Controls.Add($SupportLink) 
...
$ArcCorp141.Location = new-object System.Drawing.Point(5,160) 
...
$JavelinDaymar.Location = new-object System.Drawing.Point(5,240) 
...
$KudreOreMine.Location = new-object System.Drawing.Point(5,260) 
...
while($StartNavigation) {
    ...
    # DISPLAY RESULTS FROM DESTINATION AND SHIP PLANET COORDS
        ...
        "Current Ship Z Coord = " + [math]::Round(($CurrentZPosition / 1000), 3)
    ...
    $PosShipZ = [double]$CurrentZPosition / 1000
    ...
    #GET CLOSEST ORBITAL MARKER
    $ShipOMGSStart = ($ShipOmArray.GetEnumerator() | Sort-Object Value | Select-Object -First 1).Name
    $ShipOMForAngles = ($ShipOmArray.GetEnumerator() | Where-Object {$_.Key -ne "OM1" -AND  $_.Key -ne "OM2"} | Sort-Object Value | Select-Object -First 1).Name
    ...
    if($ShipOMGSStart -eq "OM3" -OR $ShipOMGSStart -eq "OM4"){                                  # OM on Y-axis
        $TriangleYA = $ShipRotationValueZ1 + $PoiCoordDataZ                                 # 74km
        $TriangleYB = ($DistanceShipToPlanetAlignment/1000) - [double]$PoiCoordDataY        # 148km
        $TriangleYC = [math]::Sqrt([math]::pow($TriangleYA,2) + [math]::pow($TriangleYB,2)) # 166km
        $TriangleYAlpha = [math]::Sin($TriangleYA / $TriangleYC) * 180 / [System.Math]::PI  # 24,88°

        $TriangleXA = $ShipRotationValueX1 + $PoiCoordDataX                                  # -53km
        $TriangleXB = $TriangleYB                                                            # 148km
        $TriangleXC = [math]::Sqrt([math]::pow($TriangleXA,2) + [math]::pow($TriangleXB,2))  # 157km
        $TriangleXAlpha = [math]::Sin($TriangleXA / $TriangleXC) * 180 / [System.Math]::PI * -1  # -19°

        $FinalVerticalAngle = [Math]::Round($TriangleYAlpha)
        if($TriangleXAlpha -gt 0){$FinalHorizontalAngle = [Math]::Round($TriangleXAlpha)}
        else{$FinalHorizontalAngle = [Math]::Round(360 + $TriangleXAlpha)}

        # Point nose towards the planets centre, align wings parallel to OM3-6
        Write-Host "Vertical   Angle: ${VTGreen}$FinalVerticalAngle° ${VTDefault}(Planetary, OM3,4)"
        Write-Host "Horizontal Angle: ${VTGreen}$FinalHorizontalAngle° ${VTDefault}(Planetary, OM3,4)"
    }

    if($ShipOMGSStart -eq "OM5" -OR $ShipOMGSStart -eq "OM6"){                                   # OM on X-axis
        $TriangleYA = $ShipRotationValueZ1 + $PoiCoordDataZ                                  #
        $TriangleYB = ($DistanceShipToPlanetAlignment/1000) - [double]$PoiCoordDataX         #
        $TriangleYC = [math]::Sqrt([math]::pow($TriangleYA,2) + [math]::pow($TriangleYB,2))  #
        $TriangleYAlpha = [math]::Sin($TriangleYA / $TriangleYC) * 180 / [System.Math]::PI   #

        $TriangleXA = $ShipRotationValueY1 + $PoiCoordDataY                                  # 
        $TriangleXB = $TriangleYB                                                            # 
        $TriangleXC = [math]::Sqrt([math]::pow($TriangleXA,2) + [math]::pow($TriangleXB,2))  # 
        $TriangleXAlpha = [math]::Sin($TriangleXA / $TriangleXC) * 180 / [System.Math]::PI * -1# 

        $FinalHorizontalAngle = [Math]::Round($TriangleXAlpha) *-1
        if($TriangleYAlpha -gt 0){$FinalHorizontalAngle = [Math]::Round($TriangleYAlpha)}
        else{$FinalVerticalAngle = [Math]::Round(360 + $TriangleYAlpha)}

        # Point nose towards the planets centre, align wings parallel to OM3-6
        Write-Host "Vertical   Angle: ${VTGreen}$FinalVerticalAngle° ${VTDefault}(Planetary, OM5,6)"
        Write-Host "Horizontal Angle: ${VTGreen}$FinalHorizontalAngle° ${VTDefault}(Planetary, OM5,6)"
    }

    if($ShipOMGSStart -eq "OM1" -OR $ShipOMGSStart -eq "OM2"){                                  # OM on Z-axis
        $TriangleYA = $ShipRotationValueX1 + $PoiCoordDataX                                  #
        $TriangleYB = ($DistanceShipToPlanetAlignment/1000) - [double]$PoiCoordDataZ         #
        $TriangleYC = [math]::Sqrt([math]::pow($TriangleYA,2) + [math]::pow($TriangleYB,2)) #
        $TriangleYAlpha = [math]::Sin($TriangleYA / $TriangleYC) * 180 / [System.Math]::PI   #
        ...
        # Point nose towards the planets centre, align wings parallel to OM5 & 6
        # Should = 347 & 16
        # Results = 330 & 29°
        Write-Host "Vertical   Angle: ${VTGreen}$FinalVerticalAngle° ${VTDefault}(Planetary, OM1,2)"
        Write-Host "Horizontal Angle: ${VTGreen}$FinalHorizontalAngle° ${VTDefault}(Planetary, OM1,2)"
    }
