...
# cd "C:\Users\marcel\Desktop\StarCitizen Tools\Projekt Jericho (3D Navigation)_V6\functions"
...
while($StartNavigation) {
    Start-Sleep -Milliseconds 10            # IF THIS LINE IS NOT PRESENT, CPU USAGE WILL CONSUME A FULL THREAD, AND SCRIPTS MIGHT GET UNRESPONSIVE 
    #Get ClipboardContents and Get Current Date/Time
    $ClipboardContainsCoordinates, $CurrentXPosition, $CurrentYPosition, $CurrentZPosition, $DateTime = Get-StarCitizenClipboardAndDate $DateTimeObtained
    if($DateTime){$DateTimeObtained = $true}
