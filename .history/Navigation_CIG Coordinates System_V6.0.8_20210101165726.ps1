...
if ($UseThirdPartyTools){
    if((get-process "Showlocation_Hotkey ALT-R or LEFTCTRL+ALT_RunAsAdmin" -ea SilentlyContinue) -eq $Null){
        ...
        if($env:TERM_PROGRAM -eq "vscode"){$ScriptDir = ($psEditor.GetEditorContext().CurrentFile.Path).Split-Path $._ }
        if($psISE){$ScriptDir = Split-Path -Path $psISE.CurrentFile.FullPath}
        else{$ScriptDir = $PSScriptRoot}
        Write-host -ForegroundColor Red $ScriptDir
        #Start-Process 
        & (GMO $scriptPath) {$host.EnterNestedPrompt()}
    }
}
