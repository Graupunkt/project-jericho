...
if ($UseThirdPartyTools){
    if((get-process "Showlocation_Hotkey ALT-R or LEFTCTRL+ALT_RunAsAdmin" -ea SilentlyContinue) -eq $Null){
            #DETERMINE SCRIPT PATH WITHIN VSCODE, iSE OR POWERSHELL
            if($env:TERM_PROGRAM -eq vscode){$ScriptDir = $psEditor.GetEditorContext().CurrentFile.Path}
            if($psISE){$ScriptDir = Split-Path -Path $psISE.CurrentFile.FullPath}
            else( -eq ){$ScriptDir = $PSScriptRoot}
            Write-host -ForegroundColor Red "split-path $psEditor.GetEditorContext().CurrentFile.Path"
         }
        Start-Process 
