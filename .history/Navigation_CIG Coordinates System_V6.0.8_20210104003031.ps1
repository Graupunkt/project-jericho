...
foreach ($Tool in $script:ListOfToolsToStart){
    ...
    if(get-process $3rdPartyFileName -ea SilentlyContinue){
        $test = get-process | Where-Object{$_.path -contains "*$($3rdPartyFileName)"
        $test.path | Where-Object{$_.path -contains "*$($3rdPartyFileName)"
        }
    }
