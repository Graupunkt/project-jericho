...
$ObjectContainerData = @{                                                                               #if X - offset, raise value
    ...
    Crusader            = "-18962176000;        -2664959999.99999;  0;                  0;              0;11733500              11733750;           Stanton"   #Stanton2
    ...
    Yela                = "-19022916799;         -19022916799;      0;                  1.8200001;      217.57101;              455483;             Stanton"   #Stanton2c
    StantonStar         = "136049870;           -2613996.152;       2923345368;         0;              0;                      2500000;            Stanton"   #Stanton
    #REST STOPS
...
while($StartNavigation) {
    ...
    foreach ($ObjectContainer in $ObjectContainerData.GetEnumerator()){
        ...
        Write-Host "Current OC $($ObjectContainer.Name) X=$WithinX Y=$WithinX Z=$WithinX"
        Write-Host "Stanton Coords $CurrentXPosition $CurrentYPosition $CurrentZPosition"
        Write-Host "ObjectContainer $ObjectContainerX $ObjectContainerX $ObjectContainerZ"
        Write-Host "$script:CurrentDetectedObjectContainer "
    }

    $CurrentPlanetaryXCoord = [math]::Round($ShipRotationValueX1, 3)
    $CurrentPlanetaryYCoord = [math]::Round($ShipRotationValueY1, 3)
    $CurrentPlanetaryZCoord = [math]::Round($ShipRotationValueZ1, 3)
    ...
    if($ShipOMClosest -eq "OM3" -OR $ShipOMClosest -eq "OM4"){     
        ...
        Write-Host "DESTINATION FROM OM3/4"
    ...
    if($ShipOMClosest -eq "OM5" -OR $ShipOMClosest -eq "OM6"){       
        ...
        Write-Host "DESTINATION FROM OM5/6"
        Write-Host "Planet: ${VTGreen}$($SelectedPlanet.Name)${VTDefault}, Orbital Marker: ${VTGreen}$ShipOMClosest${VTDefault}, Alignment: ${VTGreen}Planet Centre"
    ...
    if($ShipOMClosest -eq "OM2" -OR $ShipOMClosest -eq "OM1"){                                      
        ...
        Write-Host "DESTINATION FROM OM1/2"
