...
foreach ($Checkbox in $script:MainForm.Controls){
    foreach ($Checkbox.Controls.Name
}
...
#BUTTONS
if ($script:MainForm.Controls[0].Controls[0].Checked -eq $true){$script:test = "test"}
#if ($test){Write-Host "test successful"}