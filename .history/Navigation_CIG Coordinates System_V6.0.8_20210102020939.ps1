...
while($StartNavigation) {
    ...
    #$PlanetOMRadius = ($HashtableOmRadius.GetEnumerator() | Where-Object {$_.Name -eq "$CurrentPlanet"}).Value
    $PosShipX = [double]$PoiCoordDataX * 1000
    $PosShipY = [double]$PoiCoordDataY * 1000
    $PosShipZ = [double]$PoiCoordDataZ * 1000
    $ShipOM1 = [math]::Pow(([math]::Pow("$PosX","2") + [math]::Pow("$PosY","2") + [math]::Pow($PosZ-$PlanetOMRadius,"2")),1/2)
    $ShipOM1 = [math]::Round($OM1)
    $ShipOM2 = [math]::Pow(([math]::Pow("$PosX","2") + [math]::Pow("$PosY","2") + [math]::Pow($PosZ-(-$PlanetOMRadius),"2")),1/2)
    $ShipOM2 = [math]::Round($OM2)
    $ShipOM3 = [math]::Pow(([math]::Pow("$PosX","2") + [math]::Pow($PosY-$PlanetOMRadius,"2") + [math]::Pow($PosZ,"2")),1/2)
    $ShipOM3 = [math]::Round($OM3)
    $ShipOM4 = [math]::Pow(([math]::Pow("$PosX","2") + [math]::Pow($PosY-(-$PlanetOMRadius),"2") + [math]::Pow($PosZ,"2")),1/2)
    $ShipOM4 = [math]::Round($OM4)
    $ShipOM5 = [math]::Pow(([math]::Pow($PosX-$PlanetOMRadius,"2") + [math]::Pow("$PosY","2") + [math]::Pow($PosZ,"2")),1/2)
    $ShipOM5 = [math]::Round($OM5)
    $ShipOM6 = [math]::Pow(([math]::Pow($PosX-(-$PlanetOMRadius),"2") + [math]::Pow("$PosY","2") + [math]::Pow($PosZ,"2")),1/2)
    $ShipOM6 = [math]::Round($OM6)
    ...
    if($ClosestQM -eq "OM3" -OR $OMForAngles -eq "OM4"){                                  # OM on Y-axis
        ...
        $TriangleXAlpha = [math]::Sin($TriangleXA / $TriangleXC) * 180 / [System.Math]::PI   # -19°

        Write-Host -ForegroundColor Red "Angle for Daymar OM3/4"
        $TriangleYAlpha
        $TriangleXAlpha
    }

    if($ClosestQM -eq "OM5" -OR $OMForAngles -eq "OM6"){                                   # OM on X-axis
        ...
        $TriangleXAlpha = [math]::Sin($TriangleXA / $TriangleXC) * 180 / [System.Math]::PI   # 

        Write-Host -ForegroundColor Red "Angle for Daymar OM5/6"
        $TriangleYAlpha
        $TriangleXAlpha
    }

    if($ClosestQM -eq "OM1" -OR $OMForAngles -eq "OM2"){                                  # OM on Z-axis
        ...
        Write-Host -ForegroundColor Red "Angle for Daymar OM1/2"
        $TriangleYAlpha
        $TriangleXAlpha
    }
