...
foreach ($Tool in $script:ListOfToolsToStart){
    $3rdPartyFileName = switch ($Tool){
    "ToolAntilogoff" {"Autoit Scripts\AntiLogoffScript_V2.ahk"}
    "ToolShowLocation" {"Showlocation_Hotkey ALT-R or LEFTCTRL+ALT_RunAsAdmin.exe"}
    "ToolShowOnTop" {"WindowTop.exe"}
    "ToolAutorun" {"Autoit Scripts\autorun.ahk"}
    }
    Start-process -FilePath "$ScriptDir\$3rdPartyFileName"
    Write-Host "Starting $ScriptDir\$3rdPartyFileName"
