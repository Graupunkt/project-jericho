...
while($StartNavigation) {
    ...
    if(!$Powershellv5Legacy){
        ...
        #Combine String
        $VTRed = "$EscapeCharacter[${VTFontDarkGray}m"
        $VTYellow = "$EscapeCharacter[${VTFontDarkGray}m"
        $VTGreen = "$EscapeCharacter[${VTFontDarkGray}m"
        $VTBlue = "$EscapeCharacter[${VTFontDarkGray}m"
        $VTGray = "$EscapeCharacter[${VTFontDarkGray}m"
    ...
    @{
        Label = "$($VTDarkGray)Distance";                                  #NAME OF RESULTHEADING
    ...
    @{
        ...
        Expression = {switch ($_.Delta){
            ...
            default { $color = $VTDarkGray }          # COLOR GRAY IF NOTHING CHANGED
        }
