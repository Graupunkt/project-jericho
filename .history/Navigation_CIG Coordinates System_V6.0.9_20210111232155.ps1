    ...
    $LoadButton.Add_Click({
        foreach ($Box in $script:MainForm.Controls){
            foreach ($Checkbox in $Box.Controls){
                #LOAD QMS FOR JERICHO
                if($Checkbox.Name -like "*Station - INS-Jericho" -AND $Checkbox.Checked -eq $true){
                    if($Checkbox.Name -like "*Lagrange - ARC-L3"){$Checkbox.ForeColor = [System.Drawing.Color]::Green}
                }
            }
        }
    })
