...
function Get-StarCitizenClipboardAndDate{ 
    param ($DateTime) #Required to fetch the ntp server only once
    ...
    if ($ClipboardContentRAW -like '*Coordinates: x:*'){
        ...
        if($CurrentXPosition -ne $PreviousXPosition -or $CurrentYPosition -ne $PreviousYPosition -or $CurrentZPosition -ne $PreviousZPosition -AND $DateTime -gt 1){
            $NTPServer = "ptbtime1.ptb.de"
            $NTPServer = @("ptbtime1.ptb.de","ptbtime2.ptb.de","ptbtime3.ptb.de")  #Array of multiple servers, in case of multuple users or starts of the script,
