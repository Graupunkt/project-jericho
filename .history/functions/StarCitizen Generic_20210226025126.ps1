...
function Get-StarCitizenClipboardAndDate{ 
    param ($FirstRun) #Required to fetch the ntp server only once
    ...
    if ($ClipboardContentRAW -like '*Coordinates: x:*'){
        ...
        if($CurrentXPosition -ne $PreviousXPosition -or $CurrentYPosition -ne $PreviousYPosition -or $CurrentZPosition -ne $PreviousZPosition -AND $DateTime -eq $true){
