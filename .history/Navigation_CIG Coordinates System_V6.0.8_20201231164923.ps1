...
while($StartNavigation) {
    ...
    elseif($script:PlanetaryPoi){
        ...
        if ($UseTestdata){
            $UTCServerTime = [DateTime]::ParseExact('31.12.2020 14:59:00','dd.MM.yyyy HH:mm:ss',$null)
    ...
    if ($UseTestdata -and $ScriptLoopCount -lt 2){
    ...
    $CurrentShipPosition = "$CurrentXPosition;$CurrentYPosition;$CurrentZPosition"
    $CurrentShipPositionX = ($CurrentShipPosition -Split ";")[0]
    $CurrentShipPositionY = ($CurrentShipPosition -Split ";")[1]
    $CurrentShipPositionZ = ($CurrentShipPosition -Split ";")[2]
    $RevertedPlanetDataX = ($FinalPlanetDataX - $CurrentShipPositionX)
    $RevertedPlanetDataY = ($FinalPlanetDataY - $CurrentShipPositionY)
    $RevertedPlanetDataZ = ($FinalPlanetDataZ - $CurrentShipPositionZ)
    Write-Host -ForegroundColor Cyan "Planet Internal X = $([math]::Truncate($RevertedPlanetDataX/1000).ToString('N0')+"km")"
    Write-Host -ForegroundColor Cyan "Planet Internal Y = $([math]::Truncate($RevertedPlanetDataY/1000).ToString('N0')+"km")"
    Write-Host -ForegroundColor Cyan "Planet Internal Z = $([math]::Truncate($RevertedPlanetDataZ/1000).ToString('N0')+"km")"
    Write-Host -ForegroundColor Cyan "Site = Wolf Point, X = -4km, Y = 279km, Z = 95km, OM-Radius = 429km, OM3-Wolf = 178km"
