...
function Create_DynamicFormMainframe{
    ...
    $AllDataGroups = Get-Variable -Include "DataGroup*"
    $FinalGroupsForForm = @()
    foreach ($DataGroup in $AllDataGroups){
        $FinalGroupsForForm += New-Object psobject -Property @{
            Groups = $DataGroup.Name.Replace("DataGroup","")
            Options = $DataGroup.Value
        }
    }
    ...
    foreach ($Group in $FinalGroupsForForm){
        ...
        $GroupBox_Drawing_Size.Height = $MainFormVerticalSpace + 10
