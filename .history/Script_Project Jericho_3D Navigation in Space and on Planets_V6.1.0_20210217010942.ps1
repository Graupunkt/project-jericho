...
while($StartNavigation) {
    ...
    if ($pressed) { 
        ...
        #IF ON A PLANET
        if($script:CurrentDetectedObjectContainer){$line_to_write = $NewName + ';' + $CurrentDetectedObjectContainer + ';' + $CurrentPlanetaryXCoord + ';' +  $CurrentPlanetaryYCoord + ';' + $CurrentPlanetaryZCoord}
        if($script:CurrentDetectedObjectContainer){$line_to_write = "$SystemName$NewName;$CurrentDetectedObjectContainer;$CurrentPlanetaryXCoord;$CurrentPlanetaryYCoord;$CurrentPlanetaryZCoord"}
        #IF IN SPACE
