...
if($debug){
    Get-Variable -Exclude debug | Clear-Variable
    Get-Variable -Scope GLOBAL -Name "QMDistanceGreen" | Select-Object -ExpandProperty Name, Description
    | Where-Object { $_.Description -eq 'MyVars' }
    Remove-Module *
    $error.Clear()
}
