...
### DATAGROUPS ###
##################
#All Arrays beginning with DataGroup are presented in Frontend
...
$DataGroupHurstonHurtLocker = @(
    "HHL - 2951"
)

#All Other Arrays are used in backend to calculate stuff
$PlanetData = @{
    ...
    Cellin      = ";;;;;380520"     #Stanton2a
    Clio        = ";;;;;489650"     #Stanton4b
    Crusader    = ";;;;;11733500"   #Stanton2
    Daymar      = "-18930539.54;-2610158.765;0;2.4800000;29.88665;430018"   #Stanton2b
...
function Create_DynamicFormMainframe{
    ...
    $CustomWidth = 720
    $CustomHeight = 20
    $DistanceTabsToTop = 150
    $script:MainFormVerticalSpace = 10
    ...
    $script:MainFormHeight = $script:MainFormVerticalSpace + $script:MainFormVerticalBorder + $NumberFormMostOptions * $OptionHeight + $CustomHeight + $DistanceTabsToTop
    ...
    $script:MainForm.Text = "Window Title"                                                  # Window Title
    ...
    $script:DaymarRallyTab = New-Object System.Windows.Forms.TabPage
    $script:Stanton7Tab = New-Object System.Windows.Forms.TabPage
    $script:HurstonHurtLockerTab = New-Object System.Windows.Forms.TabPage

    #Tab Control 
    $tabControl.DataBindings.DefaultDataSourceUpdateMode = 0
    $System_Drawing_Point = New-Object System.Drawing.Point
    $System_Drawing_Point.X = 0
    $System_Drawing_Point.Y = $DistanceTabsToTop
    ...
    #Daymar Rally Tab
    $DaymarRallyTab.DataBindings.DefaultDataSourceUpdateMode = 0
    $DaymarRallyTab.UseVisualStyleBackColor = $True
    $DaymarRallyTab.Name = "Daymar Rally"
    $DaymarRallyTab.Text = "Daymar Rally"
    $tabControl.Controls.Add($script:DaymarRallyTab)

    #Stanton 7 Tab
    $Stanton7Tab.DataBindings.DefaultDataSourceUpdateMode = 0
    $Stanton7Tab.UseVisualStyleBackColor = $True
    $Stanton7Tab.Name = "Stanton 7"
    $Stanton7Tab.Text = "Stanton 7"
    $tabControl.Controls.Add($script:Stanton7Tab)

    #Hurston Hurt Locker Tab
    $HurstonHurtLockerTab.DataBindings.DefaultDataSourceUpdateMode = 0
    $HurstonHurtLockerTab.UseVisualStyleBackColor = $True
    $HurstonHurtLockerTab.Name = "Hurston Hurt Locker"
    $HurstonHurtLockerTab.Text = "Hurston Hurt Locker"
    $tabControl.Controls.Add($script:HurstonHurtLockerTab)
    ...
    foreach ($Group in $FinalGroupsForForm){
        ...
        #ADD GORUPBOXES TO RESPECTIVE TABS (USE DATAGROUP NAMES)
        switch -wildcard ($groupBox.Name){
            "*DaymarRally*"          {$script:DaymarRallyTab.Controls.Add($groupBox)}
            "*Stanton7*"             {$script:Stanton7Tab.Controls.Add($groupBox)}
            "*HurstonHurtLocker*"    {$script:HurstonHurtLockerTab.Controls.Add($groupBox)}
        }
    ...
    $LoadButton.Add_Click({
        foreach ($Box in $script:DaymarRallyTab.Controls){
    ...
    #$SupportLink.DoubleClickEnabled = $true
    $SupportLink.Enabled = $true
    $SupportLink.LinkColor = "black"
    #$SupportLink.Click("")

    #$SupportLink.add_Click={[System.Diagnostics.Process]::Start($SupportLink.Text)}
    ...
    ### TRACKING ID ###
    ##########################
    $TextBoxTrackingID = New-Object System.Windows.Forms.GroupBox
    $TextBoxTrackingID.Location = New-Object System.Drawing.Size(500,5) 
    $TextBoxTrackingID.size = New-Object System.Drawing.Size(480,120) 
    $TextBoxTrackingID.text = "Tracking ID:" 
    $MainForm.Controls.Add($TextBoxTrackingID) 

    # X INPUT
    $script:TrackingIDData = New-Object System.Windows.Forms.TextBox 
    $script:TrackingIDData.Location = new-object System.Drawing.Point(20,25) 
    $script:TrackingIDData.size = New-Object System.Drawing.Size(440,20)  
    $script:TrackingIDData.Text = ""
    $TextBoxTrackingID.Controls.Add($script:TrackingIDData)
...
#COLLECT ALL TABS
$CollectionOfTabs = @()
$CollectionOfTabs  += $script:DaymarRallyTab
$CollectionOfTabs  += $script:Stanton7Tab
$CollectionOfTabs  += $script:HurstonHurtLockerTab

#DEFINE VARIABLES AND ACTIONS FOR EACH BUTTON
foreach ($Box in $CollectionOfTabs){
    if($Box.Name -like "*DaymarRally"){
        foreach ($Checkbox in $Box.Controls){
            ...
            if($Checkbox.Name -like "*DaymarRally - 2950*" -AND $Checkbox.Checked){$script:DaymarRally2950Selected = $true}
        }
    }
}
...
#DAYMAR RALLY 2950
$DaymarRally2950Checkpoints = @()
if($script:DaymarRally2950Selected){
    #CHECKPOINT 1
    $properties = @{
        CheckpointID        = 1
        CheckpointPlanet    = "Daymar"
        CheckpointName      = "Shubin Mining SCD1"
        CheckpointType      = "Start"
        CheckpointXCoord    = "126.025172040445"
        CheckpointYCoord    = "-149.58146894637"
        CheckpointZCoord    = "221.25334404046"
    }
    $DaymarRally2950Checkpoints += New-Object psobject -Property $properties

    #CHECKPOINT 2
    $properties = @{
        CheckpointID        = 2
        CheckpointPlanet    = "Daymar"
        CheckpointName      = "Eager Flats Aid Shelter"
        CheckpointType      = "Checkpoint"
        CheckpointXCoord    = "200.545063565929"
        CheckpointYCoord    = "-104.917273995152"
        CheckpointZCoord    = "190.096594760393"
    }
    $DaymarRally2950Checkpoints += New-Object psobject -Property $properties
    ...
    $properties = @{
        CheckpointID        = 3
        CheckpointPlanet    = "Daymar"
        CheckpointName      = "Wolf Point Aid Shelter"
        CheckpointType      = "Finish"
        CheckpointXCoord    = "-4.41978284451699"
        CheckpointYCoord    = "279.066633187614"
        CheckpointZCoord    = "95.7326590587272"
    }
    $DaymarRally2950Checkpoints += New-Object psobject -Property $properties
    $DaymarRally2950Checkpoints | Select-Object -Property CheckpointID,CheckpointType,CheckpointName,CheckpointPlanet,CheckpointXCoord,CheckpointYCoord,CheckpointZCoord
}
...
# MOVE WINDOWS TO SPECIFIC POSITION AND SET WINDOW SIZE
...
while($StartNavigation) {
    ...
    ####################################
    ### Update Distance Tracker Form ###
    ####################################
    if($TrackingIDData.Text){
        Write-Host -ForegroundColor Green "ID = $($script:TrackingIDData.Text)"
    }
    else {
        Write-Host -ForegroundColor Red "No Tracking-ID Enterered"
    }
        Pause
