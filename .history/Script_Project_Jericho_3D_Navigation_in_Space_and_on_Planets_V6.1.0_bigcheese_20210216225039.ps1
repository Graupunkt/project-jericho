...
$PointsOfInterestOnPlanetsData = @{
    ...
    "Bennyhenge (Yela)"                     = "Yela;-626.649;135.683;-1.819"
...
#Load own_pois
$file_data = Get-Content own_pois.txt
foreach ($newpoi in $file_data) {
    $newpoi_entry = $newpoi.Split("=")
    $PointsOfInterestOnPlanetsData.add($newpoi_entry[0], $newpoi_entry[1])
    $DataGroupDestinations += $newpoi_entry[0]
}
$PointsOfInterestOnPlanetsData
$DataGroupDestinations
...
foreach ($Box in $script:MainForm.Controls){
    if($Box.Name -like "*Destinations"){
        foreach ($Checkbox in $Box.Controls){
            ...
            if($Checkbox.Name -like "*ppoi*" -AND $Checkbox.Checked){$script:PlanetaryPoi = $true}
...
while($StartNavigation) {
    ...
    if ($pressed) { 
        $NewName = Read-Host -Prompt 'Input the name of the POI: '
        if($script:CurrentDetectedObjectContainer){
           #    "jumptown (Yela)"                       = "Yela;-240.325;15.776;-200.406"
           $line_to_write = $NewName + '=' + $CurrentDetectedObjectContainer + ';' + $CurrentPlanetaryXCoord + ';' +  $CurrentPlanetaryYCoord + ';' + $CurrentPlanetaryZCoord
        }
        else {
           $line_to_write = $NewName + '=n/a;' + $CurrentXPosition + ';' +  $CurrentYPosition + ';' + $CurrentZPosition
        }
        $line_to_write  >> 'own_pois.txt' 
        Write-Host "... saved to own_pois.txt"
    }
    ...
    if($ShipOMClosest -eq "OM2" -OR $ShipOMClosest -eq "OM1"){                                      
        ...
        Write-Host "Alignment Nose: ${VTGreen}Planet Centre${VTGreen}, Wings: ${VTGreen}OM5-6${VTDefault}, Top: ${VTGreen}OM-3"
        Write-Host "Turret Angle: ${VTGreen}$FinalVerticalAngle°${VTDefault} (Vertical)"
        Write-Host "Turret Angle: ${VTGreen}$FinalHorizontalAngle°${VTDefault} (Horizontal)"
    }
       ...
       Write-Host "-----------------------"
       $ausgabe_prev = $ausgabe
       $ausgabe = CalcEbenenwinkel $PreviousPlanetaryXCoord $PreviousPlanetaryYCoord $PreviousPlanetaryZCoord $CurrentPlanetaryXCoord $CurrentPlanetaryYCoord $CurrentPlanetaryZCoord $PoiCoordDataX $PoiCoordDataY $PoiCoordDataZ
       $ausgabe = [math]::Round($ausgabe)
       $distanz_prev = $distanz
       $distanz = CalcDistance3d $CurrentPlanetaryXCoord $CurrentPlanetaryYCoord $CurrentPlanetaryZCoord $PoiCoordDataX $PoiCoordDataY $PoiCoordDataZ
       $distanz = [math]::Pow("$distanz","2")
       if ($ausgabe -le $ausgabe_prev) {
        Write-Host "Angle to Destination: $ausgabe Tendency: ${VTGreen}good${VTDefault}" 
       }
       else {
        Write-Host "Angle to Destination: $ausgabe Tendency: ${VTRed}bad${VTDefault}" 
       }
       if ($distanz -le $distanz_prev) {
           Write-Host "Distanz: $distanz km - Tendency: ${VTGreen}wir kommen naeher${VTDefault}"
       }
       else {
           Write-Host "Distanz: $distanz km - Tendency: ${VTRed}wir ENTFERNEN uns${VTDefault}"
       }
