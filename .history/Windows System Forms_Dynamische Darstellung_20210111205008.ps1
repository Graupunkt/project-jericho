...
function Create_DynamicFormMainframe{
    ...
    $MainForm_Drawing_Size.Height = $MainFormHeight + $MainFormVerticalBorder + $MainFormVerticalBorder * 8
    ...
    foreach ($Group in $FinalGroupsForForm){
        ...
        foreach ($Option in $Group.Options){
            ...
            $Option_Drawing_Size.Width = $OptionSpacer + (($OptionCounter -1) * $OptionWidth)
            $Option_Drawing_Size.Height = $OptionSpacer + $OptionHeight
            $OptionBox.size = $Option_Drawing_Size
            #SET SIZE OF OPTION
