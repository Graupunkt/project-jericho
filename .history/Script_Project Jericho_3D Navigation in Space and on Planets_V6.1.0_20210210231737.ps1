...
$ObjectContainerData = @{                                            #If x has a negative offset, raise Rotation Adjustment     OrbitalMarkerRadius is also used to determine if the player is within the object container for now
    ...
    Yela                = "-19022916799;        -2613996152;        0;                  1.8200001;      217.57101;              455483;             Stanton"   #Stanton2c #X and Y values where split, so adjust by -90° from 217.57101 to127.57101
...
$PointsOfInterestOnPlanetsData = @{
    ...
    "Bennyhenge (Yela)"                     = "Yela;-627.017;133.972;-1.811"
}
...
while($StartNavigation) {
    ...
    if ($PreviousXPosition -ne $null) {
        ...
        Write-Host "Course Deviation (Space) = ${FAcolor}$("$FinalAngle°") ${VTDefault}(Previous: $PreviousAngle °)"
    ...
    #$c1 = CalcDistance3d $DestCoordDataX $DestCoordDataY $DestCoordDataZ $PreviousXPosition $PreviousYPosition $PreviousZPosition
    $c2Local = CalcDistance3d $DestCoordDataX $DestCoordDataY $DestCoordDataZ $CurrentXPosition $CurrentYPosition $CurrentZPosition
    ...
    Write-Host "Course Deviation (Space) = ${FALcolor}$("$FinalAngleLocal") ${VTDefault}(Previous: $PreviousAngle °)"
    Write-Host "Course Diav : $FinalAngleLocal"
    Write-Host ""
