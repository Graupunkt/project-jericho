...
function Create_DynamicFormMainframe{
    ...
    $LoadButton.Location = New-Object System.Drawing.Size(30,10) 
    ...
    $LoadButton.Add_Click({
        foreach ($Box in $script:MainForm.Controls){
            foreach ($Checkbox in $Box.Controls){
                if($Checkbox.Name -like "*OptionA1" -AND $Checkbox.Checked){$Checkbox.Checked = $true}
            }
        }
    })
...
#CREATE A LIST OF FUNCTIONS THAT ARE RUN
$ExecutionList = @()
foreach ($Box in $script:MainForm.Controls){
    foreach ($Checkbox in $Box.Controls){
...
$ExecutionList
# RUN EACH DEFINED FUNCTION IN EXECUTION LIST
#$ExecutionList | ForEach { Invoke-Expression $_ }


#DEFINE VARIABLES AND ACTIONS FOR EACH BUTTON
foreach ($Box in $script:MainForm.Controls){
    foreach ($Checkbox in $Box.Controls){
        if($Checkbox.Name -like "*OptionA1" -AND $Checkbox.Checked){Write-Host "Check A1"}
        if($Checkbox.Name -like "*OptionB1" -AND $Checkbox.Checked){Write-Host "Check B1"}
    }
}
