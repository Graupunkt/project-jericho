...
switch($script:CurrentDestination){
    {$_ -in $PointsOfInterestOnPlanetsData.Name} {Write-Host "TEST",$script:PlanetaryPoi = $true}
...
while($StartNavigation) {
    ...
    if($script:PlanetaryPoi){
        ...
        $test = $PointsOfInterestOnPlanetsData.GetEnumerator() | Where-Object { $_.Name -eq $script:CurrentDestination }
        ...
        Write-Host "0 $script:CurrentDestination"
        Write-Host "1 $SelectedDestination"
        Write-Host "2 $test"
