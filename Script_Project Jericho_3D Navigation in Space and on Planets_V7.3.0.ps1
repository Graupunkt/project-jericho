...
while($StartNavigation) {
    ##################################################################################################
    ### CALCULATIONS AND STUFF THAT DOES NOT NEED ANY PLAYER COORDINATES OR A SELECTED DESTINATION ###
    ##################################################################################################
    #DEBUG - Generic Stuff for debugging
    if($global:debug -eq $true){
        #Measure Loop Duration
        $MeasureLoopDuration.Stop()
        Write-Host "$($OrangeForeColor)- LoopDuration = $($MeasureLoopDuration.Elapsed)"
        $MeasureLoopDuration = [system.diagnostics.stopwatch]::startNew()
    ...
    ### HOTKEY AND BUTTON EVENTS ###
    # IF FORM IS CLOSED ON TOP RIGHT
    if($global:exitflag){
        #Kills all child processes, since runspace can not be stopped from outside
        if($global:debug){Write-Host "$($OrangeForeColor)- Exit Button Pressed"}
        Stop-Transcript                         #STOP WRITING TO LOGFILE
        start-process cmd -WindowStyle Minimized -argumentlist '/c timeout /t 3 & taskkill /F /IM pwsh.exe' #KILL ALL PWSH.EXE LIKE OVERLAY
        $formProjectJericho.Close()             #Close Main Application
        $rs.Close()                             #Close Runspace Main Application
        $PSinstanceHotkey.RunSpace.Dispose()    #Close Runspace Hotkeys
        $PSinstanceHotkey.Dispose()             #Close Runspace Hotkeys
        $RunspacePool.Close()                   #Closes all other Runspaces that are part of a runspace pool
        $RunspacePool.Dispose()                 #Closes all other Runspaces that are part of a runspace pool
        exit
    }

    if($global:ShowPoiSelection){               #Check from MainApp Runspace, if SELECT Button was clicked
        if($global:debug){Write-Host "$($OrangeForeColor)- Select Destination Button Pressed"}
    ...
    ##############################################################
        ...
        #}
            ...
            $OmDestArray = @{}
            $OmDestArray.add("OM1",$OM1Dest);$OmDestArray.add("OM2",$OM2Dest);$OmDestArray.add("OM3",$OM3Dest);$OmDestArray.add("OM4",$OM4Dest);$OmDestArray.add("OM5",$OM5Dest);$OmDestArray.add("OM6",$OM6Dest)

            #GET CLOSEST ORBITAL MARKER
            $OMGSDestStart = ($OmDestArray.GetEnumerator() | Sort-Object Value | Select-Object -First 1).Name
            #$OMForAngles = ($OmArray.GetEnumerator() | Where-Object {$_.Key -ne "OM1" -AND  $_.Key -ne "OM2"} | Sort-Object Value | Select-Object -First 1).Name
            $OMGSDistanceToDestination2 = ($OmDestArray.GetEnumerator() | Sort-Object Value | Select-Object -First 1).Value
    ...
    if($TabControl1.SelectedTab.Name -eq "tabpage6"){
        ...
        $textbox20.Text = $ClostestOCPlayer.Name        #Next OC
        $textbox247.Text = $OMGSDestStart               #NEXT OM
        ...
        $textbox248.Text = [math]::Truncate($OMGSDistanceToDestination2/1000).ToString('N0')            #Next OM Distance
