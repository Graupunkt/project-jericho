...
$script:CustomCoordsProvided 
$script:HudCrosshair 


$script:MainForm.Controls | Select-Object -Property Name | Out-GridView
$script:MainForm.Controls[6] # Stanton Coordinates
$script:MainForm.Controls[5].Controls[4].Checked # 3rd Party Tools, HUD Corrhair

foreach ($Box in $script:MainForm.Controls){
    Write-Host -ForegroundColor Red $Box.Name
    switch -Wildcard ($Box.Name){
        "*Stanton Coordinates*"        {Write-Host $Box.Name}
        "*3rd Party Tools Autostart*"  {Write-Host $Box.Name}
    }
}
#>
