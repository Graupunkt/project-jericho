...
while($StartNavigation) {
    ...
    if($CurrentDestination -ne $null){
        ...
        if ($ResetPressed) {
            ...
            if($console){Write-Host "Rotation calibrated from $($CurrentDetectedOCADX)° to $($FinalRotationAdjustment)° by $RotationSpeedAdjustment, pls restart script"}
    ...
    ##############################################################
        ...
        if($script:PlanetaryPoi){
            ...
            #OM COLORS
            switch ($OM1){
                {($_..$OM1D).Count -le 1}  { $textbox158.Forecolor = [System.Drawing.Color]::FromArgb(50,250,50); break }
                {($_..$OM1D).Count -le 10} { $textbox158.Forecolor = [System.Drawing.Color]::FromArgb(200,200,50); break }
                default                    { $textbox158.Forecolor = [System.Drawing.Color]::FromArgb(120,120,120)}
            }
            switch ($OM2){
                {($_..$OM2D).Count -le 1}  { $textbox156.Forecolor = [System.Drawing.Color]::FromArgb(50,250,50); break }
                {($_..$OM2D).Count -le 10} { $textbox156.Forecolor = [System.Drawing.Color]::FromArgb(200,200,50); break }
                default                    { $textbox156.Forecolor = [System.Drawing.Color]::FromArgb(120,120,120)}
            }
            switch ($OM3){
                {($_..$OM3D).Count -le 1}  { $textbox154.Forecolor = [System.Drawing.Color]::FromArgb(50,250,50); break }
                {($_..$OM3D).Count -le 10} { $textbox154.Forecolor = [System.Drawing.Color]::FromArgb(200,200,50); break }
                default                    { $textbox154.Forecolor = [System.Drawing.Color]::FromArgb(120,120,120)}
            }
            switch ($OM4){
                {($_..$OM4D).Count -le 1}  { $textbox153.Forecolor = [System.Drawing.Color]::FromArgb(50,250,50); break }
                {($_..$OM4D).Count -le 10} { $textbox153.Forecolor = [System.Drawing.Color]::FromArgb(200,200,50); break }
                default                    { $textbox153.Forecolor = [System.Drawing.Color]::FromArgb(120,120,120)}
            }
            switch ($OM5){
                {($_..$OM5D).Count -le 1}  { $textbox150.Forecolor = [System.Drawing.Color]::FromArgb(50,250,50); break }
                {($_..$OM5D).Count -le 10} { $textbox150.Forecolor = [System.Drawing.Color]::FromArgb(200,200,50); break }
                default                    { $textbox150.Forecolor = [System.Drawing.Color]::FromArgb(120,120,120)}
            }
            switch ($OM6){
                {($_..$OM6D).Count -le 1}  { $textbox148.Forecolor = [System.Drawing.Color]::FromArgb(50,250,50); break }
                {($_..$OM6D).Count -le 10} { $textbox148.Forecolor = [System.Drawing.Color]::FromArgb(200,200,50); break }
    ...
    if($TabControl1.SelectedTab.Name -eq "tabpage6"){
        ...
        $ForeCast = [decimal]$CurrentDistanceTotal/1000 - (($DateTime - $UpdateTime).TotalSeconds * $CurrentDeltaTotal/1000)
        ...
        $textbox157.Text = [Math]::Round($OM1D, 1).ToString("0.0") #OM Distances Destination
        $textbox155.Text = [Math]::Round($OM2D, 1).ToString("0.0")
        $textbox152.Text = [Math]::Round($OM3D, 1).ToString("0.0")
        $textbox151.Text = [Math]::Round($OM4D, 1).ToString("0.0")
        $textbox149.Text = [Math]::Round($OM5D, 1).ToString("0.0")
        $textbox147.Text = [Math]::Round($OM6D, 1).ToString("0.0")
